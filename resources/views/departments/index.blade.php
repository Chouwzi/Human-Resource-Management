...
<body class="bg-gray-50 text-gray-800 antialiased min-h-screen flex flex-col md:flex-row">
    ...
    <aside class="w-full md:w-64 bg-slate-900 text-white flex flex-col shrink-0 shadow-xl">
        ...
        <nav id="sidebar-menu" class="hidden md:flex flex-col p-4 space-y-2 flex-1">
            <a href="{{ route('departments.index') }}" class="w-full flex items-center space-x-3 px-4 py-3 rounded-lg font-medium transition bg-indigo-600 text-white">
                <i class="fa-solid fa-building w-5 text-center"></i><span>Quản lý Phòng ban</span>
            </a>
            <a href="{{ route('positions.index') }}" class="w-full flex items-center space-x-3 px-4 py-3 rounded-lg font-medium text-slate-400 hover:bg-slate-800 hover:text-white transition">
    ...
    <main class="flex-1 p-4 md:p-8 overflow-x-hidden">
        <header class="flex flex-col sm:flex-row sm:items-center sm:justify-between pb-6 mb-6 border-b border-gray-200 gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 md:text-3xl">Quản lý Phòng ban</h1>
                <p class="text-sm text-gray-500 mt-1">Xem, thêm, sửa và xóa danh sách các phòng ban trong tổ chức.</p>
            </div>
            <div>
                <button onclick="openAddModal()" class="w-full sm:w-auto inline-flex items-center justify-center space-x-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg shadow-sm transition cursor-pointer">
                    <i class="fa-solid fa-plus text-sm"></i><span>Thêm Phòng ban</span>
                </button>
            </div>
        </header>
                    ...
                    <tbody class="divide-y divide-gray-200 text-sm text-gray-700">
                        @forelse($departments as $d)
                            <tr class="hover:bg-gray-50/70 transition">
                                <td class="px-6 py-4 font-mono font-medium text-indigo-600">{{ $d->code }}</td>
                                <td class="px-6 py-4 font-semibold text-gray-900">{{ $d->name }}</td>
                                <td class="px-6 py-4 text-gray-500">{{ $d->leader ?? 'Chưa bổ nhiệm' }}</td>
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium {{ $d->status === 'Hoạt động' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-amber-50 text-amber-700 border border-amber-200' }}">
                                        {{ $d->status }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-right space-x-2">
                                    <button onclick="openEditModal({{ json_encode($d) }})" class="text-slate-600 hover:text-indigo-600 p-1.5 hover:bg-gray-100 rounded transition cursor-pointer"><i class="fa-solid fa-pen-to-square"></i></button>
                                    <button onclick="triggerDelete({{ $d->id }})" class="text-slate-600 hover:text-rose-600 p-1.5 hover:bg-gray-100 rounded transition cursor-pointer"><i class="fa-solid fa-trash-can"></i></button>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-10 text-center text-gray-400">Không có dữ liệu phòng ban nào.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <div id="crud-modal" class="fixed inset-0 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4 z-40 hidden">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div class="px-6 py-4 bg-gray-50 border-b border-gray-200 flex items-center justify-between">
                <h3 id="modal-title" class="text-lg font-semibold text-gray-900">Thêm phòng ban</h3>
            ...
            <form id="modal-form" method="POST" class="p-6 space-y-4">
                ...
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase mb-2 tracking-wide">Tên phòng ban <span class="text-red-500">*</span></label>
                ...
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase mb-2 tracking-wide">Trưởng phòng</label>
                    <input type="text" id="form-leader" name="leader" class="w-full px-3.5 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 text-sm">
                </div>
                <div>
    ...
    <div id="confirm-modal" class="fixed inset-0 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4 z-40 hidden">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden p-6 text-center">
            ...
            <p class="text-sm text-gray-500 mb-6">Hành động này sẽ xóa phòng ban vĩnh viễn khỏi cơ sở dữ liệu hệ thống.</p>
    ...
    <script>
        ...
        function openAddModal() {
            form.reset();
            document.getElementById('modal-title').innerText = "Thêm phòng ban mới";
        ...
        function openEditModal(department) {
            document.getElementById('modal-title').innerText = "Cập nhật phòng ban";
            // Gán động URL edit theo chuẩn Resource của Laravel (/departments/{id})
            form.action = `/departments/${department.id}`;
            methodField.value = "PUT"; // Giả lập Method PUT trong Laravel Form
            
            document.getElementById('form-code').value = department.code;
            document.getElementById('form-name').value = department.name;
            document.getElementById('form-leader').value = department.leader || '';
            document.getElementById('form-status').value = department.status;
            modal.classList.remove('hidden');
        }
        ...
        function triggerDelete(id) {
            document.getElementById('delete-form').action = `/departments/${id}`;
